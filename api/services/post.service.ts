...
export const getPostsByAccountId = (accountId: string, viewerId?: string) => {
	return prisma.post.findMany({
		where: {
			accountId,
			OR: [
				{ private: false },
				...(viewerId && viewerId === accountId ? [{ private: true }] : []),
			],
		},
		orderBy: {
			creationDate: "desc",
...
export const getFeedPosts = async (accountId?: string) => {
	const followingIds = accountId
		? await import("./follow.service").then((m) => m.getFollowingIds(accountId))
		: [];
	...
	return prisma.post.findMany({
		where: {
			OR: [
				{ private: false },
				...(accountId ? [{ accountId: { in: followingIds } }] : []),
				...(accountId ? [{ accountId }] : []),
			],
		},
		orderBy: {
